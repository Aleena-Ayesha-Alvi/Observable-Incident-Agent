...
def apply_receipt(run, receipt):
    ...
    if not isinstance(rid, str) or not rid: raise ValueError("receiptId is required")
    action_id = safe.get("actionId")
    action = next((a for a in run["actions"] if a["actionId"] == action_id), None)
    if not action: raise ValueError("unknown actionId")
    ...
    if safe.get("approvalId") or safe.get("type") == "approval":
        ...
        if not approval or action["phase"] != "awaiting_approval": raise ValueError("approval is not pending")
        ...
        if action["phase"] != "pending": raise ValueError("action is not pending")
        status = str(safe.get("status", "success")).lower()
        if status in {"503", "http_503", "service_unavailable"} and action["attempt"] == 1:
            action["attempt"] = 2; action["callId"] = stable_id("call_", run["runId"], action["actionId"], 2)
            ...
            action["phase"] = "failed"; event = {"receiptId": rid, "actionId": action_id, "status": "failed", "attempt": action["attempt"]}
        else:
            action["phase"] = "completed"; event = {"receiptId": rid, "actionId": action_id, "status": "completed", "attempt": action["attempt"]}
    run["receiptLog"].append(event); run["actionLog"].append({"actionId": action_id, "phase": action["phase"], "attempt": action["attempt"]})
    refresh(run)
